...
\begin{theorem}
  \label{thm:ordering}
    ...
    Now fix $x\in\Lambda$.
    Consider the measures $\mu:=\mu_{\Lambda,\beta}^\zeta$ and $\mu_\pm:=\mu_{\Lambda,\beta}^{\zeta}[\blank|\{\sigma_x=\pm 1\}]$.
    Consider the Glauber dynamic $\P$ for these three measures, started from the same initial state $X_0^\mu=X_0^{\mu_\pm}=\sigma$.
...
\begin{proof}[Proof of Theorem~\ref{thm:ordering}]
    We first prove the ordering in the dynamic, and then deduce the ordering of the measures from it.
    ...
    \end{equation}
    for any $k$.
    As $k\to\infty$, these expectations converge to $\mu_-[f]$, $\mu[f]$, and $\mu_+[f]$ respectively.
\end{proof}
...
\begin{theorem}[Construction of the infinite-volume measure]
    ...
    \begin{equation}
        ...
        \langle f\rangle_{G,\beta}^+ := \mu_{G,\beta}^+[f] = \lim_{\Lambda\uparrow V} \langle f\rangle_{\Lambda,\beta}^+,
    \end{equation}
