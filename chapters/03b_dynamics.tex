...
\begin{figure}
    ...
    with the same input data $(u,\iota)$ preserves the ordering of the configurations.}
    \label{fig:mono}
\end{figure}
...
\begin{lemma}[Glauber update for conditioned measures]
    ...
    Consider a probability measure on $\Omega$ such that every state has a positive probability.
    For $\Lambda\subset V$ and $\zeta\in\{\pm1\}^\Lambda$,
    define the measure $\nu:=\mu[\blank|\{\sigma|_\Lambda=\zeta\}]$.
    Then:
    \begin{itemize}
        \item For $u\in\Lambda^\complement$, we have $R_{u,\iota}^\nu=R_{u,\iota}^\mu$ (at least on the set $\{\sigma|_\Lambda=\zeta\}$),
        \item For $u\in\Lambda$, we have $R_{u,\iota}^\nu=f_u^{\zeta_u}$ (at least on the set $\{\sigma|_\Lambda=\zeta\}$).
    \end{itemize}

    In particular, if $\mu$ is an Ising model, then $R_{u,\iota}^\nu$ is an increasing function.
\end{lemma}
