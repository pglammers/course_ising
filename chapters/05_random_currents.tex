...
\begin{itemize}
    \item $\M$ is a nonnormalised family of independent
        $\operatorname{Poisson}(\beta)$-variables $(\n_{xy})_{xy\in E}$,
...
\begin{exercise}[Poisson switching]
    ...
    \begin{enumerate}
        ...
        \item $\P[\{\text{$X_B$ is even}\}]-\P[\{\text{$X_B$ is
            odd}\}]=\P[\{\text{$X_B+X_Y=0$}\}]\geq 0$.
    \end{enumerate}
\end{exercise}
...
\begin{proof}
    ...
    \begin{align}
        ...
        \M^2[\{\widehat{\n+\m} \in \calE_B\}\cap\{\partial\n=A\Delta
        B\}\cap\{\partial\m=\emptyset\}]
        \\
        &\qquad\leq
        \M^2[\{\partial\n=A\Delta B\}\cap\{\partial\m=\emptyset\}].
    \end{align}
    For the first equality, we simply observe that $\{\partial\m=B\}\subset
    \{\widehat{\n+\m} \in \calE_B\}$
    ...
    By the random currents expansion of correlation functions
...
\begin{exercise}[Correlation functions in terms of sourceless currents]
    ...
    $4^{|V|}\M^2[\{\partial\n=\partial\m=\emptyset\}\cap(\blank)]/Z^2$ on sourceless
    random currents.
\end{exercise}
