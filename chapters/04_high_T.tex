...
also fit into this framework, see Definition~\ref{definition:ghost} and
Lemma~\ref{lemma:ghost} below.
...
\[
    ...
    =
    \frac{\sum_{\sigma\in\Omega}\sigma_A \prod_{xy\in E} e^{\beta\sigma_x\sigma_y}
    }{\sum_{\sigma\in\Omega}\prod_{xy\in E} e^{\beta\sigma_x\sigma_y} }.
\]
...
\begin{itemize}
    ...
    \item We may write $e^{\beta\sigma_x\sigma_y}=e^{-2\beta} +
        2\cdot\true{\sigma_x=\sigma_y}\sinh\beta$.
\end{itemize}
...
\begin{proof}[Proof overview of Theorem~\ref{thm:Exponential decay at high temperature}]
    ...
    \begin{itemize}
        ...
        \item One bounds the number of walks $\gamma$ of length $n$ from $x$ by
            $2d(2d-1)^{|\gamma|-1}$.
    \end{itemize}
    \qedhere
\end{proof}
...
\begin{itemize}
    ...
    \item In dimension $d=1$, Theorem~\ref{thm:Exponential decay at high temperature}
        also implies Ising's result (Theorem~\ref{thm:Ising}),
