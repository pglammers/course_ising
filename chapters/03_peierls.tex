...
\begin{exercise}[Peierls argument in the Potts model]
    ...
    and consider the probability measure $\mu_n$ defined on $\{1,\ldots,N\}^{\T_n}$
    such that
    \[
        \mu_n[\sigma]\propto \exp\beta \sum_{uv\in E(\T_n)} \begin{cases}
    ...
    to the vertices of $\T_n$, in such a way that configurations with fewer
    ...
    \emph{Hint.}
    If we consider $\sigma\in(\Z/N\Z)^{\T_n}$, then for any fixed loop $\gamma$
