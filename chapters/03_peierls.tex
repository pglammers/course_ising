...
The triangular lattice $\T$ is the nearest-neighbour graph on the vertex set $i\Z+\Z e^{i\pi/6}\subset\C$.
We write $\T_n$ for the subgraph induced by vertices at a graph distance strictly smaller than $n$ from the origin,
...
\begin{lemma}[Loop representation of the Ising model on the triangular lattice]
    ...
    \begin{enumerate}
        \item The map $\sigma\mapsto\calI(\sigma)$ is a bijection from the set of spin configurations $\sigma$ on $\bar\T_n$
...
\begin{exercise}[Peierls argument in the Potts model]
    ...
    such that
    \[
        \mu_n[\sigma]\propto \exp\beta \sum_{uv\in E(\bar\T_n)} \begin{cases}
    ...
    defined by
    \[
        S_{\gamma,k}(\sigma)_u:=\begin{cases}
