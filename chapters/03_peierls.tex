...
\begin{exercise}[Peierls argument in the Potts model]
    Now change the model: fix $N\in\Z_{\geq 2}$ and $\beta\in[0,\infty)$,
    and consider the probability measure $\mu_n$ defined on $\{1,\ldots,N\}^{\bar\T_n}$
    such that
    \[
        ...
        \end{cases}
    \]
    We think of $\mu_n$ as assigning one of $N$ distinct \emph{colours}
    to the vertices of $\bar\T_n$, in such a way that configurations with fewer
    disagreement edges are favoured.
    This model is called the \emph{$N$-state Potts model}.

    Prove that for fixed $N$, for sufficiently large $\beta$, we have
    \begin{equation}
        \label{eq:PeierlsPottsGoal}
        \inf_n\mu_n[\{\sigma_0=1\}|\{\sigma|_{\partial\T_n}\equiv 1\}]\geq \frac1N.
    \end{equation}
    ...
    \emph{Hint.}
    If we consider $\sigma\in(\Z/N\Z)^{\bar\T_n}$, then for any fixed loop $\gamma$
    and shift $k\in(\Z/N\Z)$, we can define the configuration $S_{\gamma,k}(\sigma)$
    defined by
    \[
