...
\begin{figure}[b]
    ...
    \caption{The triangular lattice graph $\bar\T_3$ with $+$ spins on the boundary.}
...
The triangular lattice $\T$ is the nearest-neighbour graph on the vertex set $i\Z+\Z
e^{i\pi/6}\subset\C$.
We write $\T_n$ for the subgraph induced by vertices at a graph distance strictly smaller
than $n$ from the origin,
and $\partial\T_n$ for the set of vertices at a graph distance of exactly $n$ from the origin.
We are interested in the Ising model $\langle\blank\rangle_{\T_n,\beta}^+$,
...
\begin{theorem}[Peierls, 1936]
    ...
    \[
        \inf_n\langle\sigma_0\rangle_{\T_n,\beta}^+>0.
    \]
    In other words, the centre spin remains positively correlated with the boundary,
    uniformly in the system size.
\end{theorem}

Before proving this theorem, we consider the combinatorics of the Ising model on the
triangular lattice.
...
Although this is no more than a visual trick, the picture suggests that $\sigma$ can be
represented by a collection of \emph{loops}.

We now make this formal.
Let $\T_n^*=(V_n^*,E_n^*)$ denote the \emph{dual graph} of $\bar\T_n$.
Its vertex set is the set of centres of triangles of $V(\bar\T_n)$.
Its edge set connects nearest-neighbour vertices.
See Figure~\ref{fig:peierls} (Right).
For any spin configuration $\sigma$ on $\bar\T_n$ which equals $+$ on $\partial\T_n$,
we define its \emph{interface} $\calI(\sigma)\subset E_n^*$ as the set of dual edges
separating two spins with a distinct value.
See Figure~\ref{fig:peierls} for such a spin configuration $\sigma$ and its interface
$\calI(\sigma)$.

\begin{figure}[b]
    \includegraphics{figures/Peierls.pdf}
    \caption{
        Left: A spin configuration $\sigma$ on $\bar\T_3$ with $+$ boundary conditions.
        The spins are represented by hexagons in such a way that they fill up the space.
        Right: The triangular lattice graph $\bar\T_3$ (in grey)
        and its planar dual $\T_3^*$ (in black).
        It also depicts the loops of the interface $\calI(\sigma)$ for the configuration
        $\sigma$ on the left.
    }
    \label{fig:peierls}
\end{figure}
...
\begin{lemma}[Loop representation of the Ising model on the triangular lattice]
    ...
    \begin{enumerate}
        \item The map $\sigma\mapsto\calI(\sigma)$ is a bijection from the set of spin
            configurations $\sigma$ on $\bar\T_n$
...
\begin{proof}
    \begin{itemize}
        \item\textbf{Proof that $\calI(\sigma)$ is a loop configuration.} It is easy to
            see from Figure~\ref{fig:peierls} that $\calI(\sigma)$
            is a loop configuration for any spin configuration $\sigma$ on $\T_n$
            that equals $+$ on $\partial\T_n$.
        \item\textbf{Proof that $\calI$ is a bijection and
            Equation~\eqref{eq:peierls_reconstruction} holds.}
            ...
            This proves that $\calI$ is a bijection, and that
            Equation~\eqref{eq:peierls_reconstruction} holds.
        \item\textbf{Proof of Equation~\eqref{eq:peierls_energy}.}
            Notice that $|\calI(\sigma)|$ equals the number of $\bar\T_n$-edges
            on which $\sigma$ disagrees.
            In particular, we have
            \[
                H(\sigma)=-\beta\sum_{u\sim v}\sigma_u\sigma_v=
                -\beta|E(\bar\T_n)| + 2\beta|\calI(\sigma)|,
            \]
...
\begin{lemma}[The energy of a loop]
    \label{lemma:Peierls_loop_energy}
    Fix $n$ and $\beta$.
...
\begin{proof}[Proof of Theorem~\ref{thm:peierls}]
    ...
    Thus, if $\calI(\sigma)$ contains a loop around $0$, then it must contain a loop of
    length at least $i$ through $e_i$ for some $i\geq 1$.
    In particular, we get
    \begin{multline}
        \mu_{\T_n,\beta}^+[\{\sigma_0=-\}]
        \\\leq
        \sum_{i=1}^\infty \mu_{\T_n,\beta}^+[\{\text{$\calI(\sigma)$ contains a loop of
        length at least $i$ through $e_i$}\}]
        \\\leq
...
\begin{figure}
    \centering
    \includegraphics{figures/Peierls_Potts.pdf}
    \caption{A loop $\gamma\subset\calI(\sigma)$ in the three-state Potts model}
    \label{fig:Peierls_Potts}
\end{figure}

\begin{exercise}[Peierls argument in the Potts model]
    Now change the model: fix $q\in\Z_{\geq 2}$ and $\beta\in[0,\infty)$,
    and consider the probability measure $\mu_n$ defined on $\{1,\ldots,q\}^{\bar\T_n}$
    such that
    \[
        \mu_n[\sigma]\propto \exp\beta \sum_{uv\in E(\bar\T_n)}
        \begin{cases}
            +1 &\text{if $\sigma_u=\sigma_v$,}
            \\
            -1 &\text{if $\sigma_u\neq\sigma_v$.}
        \end{cases}
    \]
    We think of $\mu_n$ as assigning one of $q$ distinct \emph{colours}
    to the vertices of $\bar\T_n$, in such a way that configurations with fewer
    disagreement edges are favoured.
    This model is called the \emph{$q$-state Potts model}.

    Prove that for fixed $q$, for sufficiently large $\beta$, we have
    \begin{equation}
        \label{eq:PeierlsPottsGoal}
        \inf_n\mu_n[\{\sigma_0=1\}|\{\sigma|_{\partial\T_n}\equiv 1\}]> \frac1q.
    \end{equation}

    The prove this,
    define $\calI(\sigma)$ as the dual edges of disagreement edges
    (the union of coloured cluster boundaries in Figure~\ref{fig:Peierls_Potts}).
    Notice that $\calI(\sigma)$ gives every vertex degree $0$, $2$, or $3$
    (but never $1$).
    To prove~\eqref{eq:PeierlsPottsGoal},
    one should generalise Lemma~\ref{lemma:Peierls_loop_energy}, that is, upper bound
    $\mu_n[\{\gamma\subset\calI(\sigma)\}|\{\sigma|_{\partial\T_n}\equiv 1\}]$
    for any loop $\gamma$.

    \emph{Hint.}
    If we consider $\sigma\in(\Z/q\Z)^{\bar\T_n}$, then for any fixed loop $\gamma$
    and shift $k\in(\Z/q\Z)$, we can define the configuration $S_{\gamma,k}(\sigma)$
    defined by
    \[
        S_{\gamma,k}(\sigma)_u:=
        \begin{cases}
            \sigma_u &\text{if $u$ is not surrounded by $\gamma$,}\\
            \sigma_u+k&\text{if $u$ is surrounded by $\gamma$.}
        \end{cases}
    \]
\end{exercise}
...
\begin{exercise}[Peierls argument on the square lattice]
    Fix $d=2$.
    Now let $\Lambda_n:=(-n,n)^d\cap\Z^d$ be a subset of vertices
    of the square lattice graph $\Z^2$ (with nearest-neighbour connectivity).
    Prove that for sufficiently large $\beta$, we have
    \[
        \inf_n\langle\sigma_0\rangle_{\Lambda_n,\beta}^+>0.
    \]
    \emph{Hint.}
...
%         \item If $\sigma_{(0,0)}=-$, then $\calI(\sigma)$ contains at least one
% self-avoiding loop around $(0,0)$.
%     \end{itemize}
...
%         \leq
%         \P_{\Lambda_n,\beta}^+[\{\text{$\calI(\sigma)$ contains a loop $\gamma$ around
% $(0,0)$}\}].
%     \end{equation}
...
%     \begin{claim*}
%         For any fixed loop $\gamma$, we may bound
% $\P_{\Lambda_n,\beta}^+[\{\gamma\subset\calI(\sigma)\}]\leq e^{-2\beta|\gamma|}$.
%     \end{claim*}

%     \begin{proof}[Proof of the claim]
%     We would like to define a \emph{loop erasure map}
% $\calE:\{\gamma\subset\calI(\sigma)\}\to\Omega'$,
...
%         \[
%         \P_{\Lambda_n,\beta}^+[\{\text{$\calI(\sigma)$ contains a loop of length at
% least $\ell$ through $e$}\}]
